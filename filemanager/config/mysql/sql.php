...
class pdoOperation{
	...
	//通过文件路径获取fid
	private $selectFidByPath="SELECT `fid` FROM `fmdb_file` WHERE `path`=?";

	private $getUserPrivilege="SELECT `privilege` from `fmdb_user` WHERE `uid`=?";
...
class userMgr extends pdoOperation{
	...
	function getPrivilege($uid){
		return $this->fetchOdd($this->getUserPrivilege,array($uid));
	}
