(function() {
    ...
    function BasicVolunteerController($scope, $state, VolunteerService, $mdDialog, RefreshService) {
        ...
        $scope.readCityByProvince = function(province) {
            for (var i = 0; i < $scope.provinceList.length; i++) {
                var currentProvince = $scope.provinceList[i];
                if(currentProvince.name == province) {
                    $scope.cityList = currentProvince.cityList;
                    break;
                }
            };
        }
        ...
        $scope.changeCity = function() {
            $scope.readCityByProvince($scope.volunteerUpdateInfo.province);
        }

        $scope.changeDistrictByCity = function(city) {
            for (var i = 0; i < $scope.cityList.length; i++) {
                var currentProvince = $scope.cityList[i];
                if(currentProvince.name == city) {
                    $scope.districtList = currentProvince.areaList;
                    break;
                }
            };
        }
        ...
        $scope.changeDistrict = function() {
            $scope.changeDistrictByCity($scope.volunteerUpdateInfo.city);
        }
        ...
        $scope.getVolunteerInfo = function() {
            ...
            VolunteerService.index($scope.currentVolunteerPage, 10).then(function(data) {
                ...
                if(status != '200') {
                    ...
                    $scope.volunteerInfoCount = realData.properties.count;
        ...
        $scope.confirmUpdateVolunteerInfo = function() {
            ...
            if($scope.volunteerUpdateInfo.realname == null) {
                alert('姓名不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.sex == null) {
                alert('性别不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.province == null) {
                alert('省不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.city == null) {
                alert('市不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.district == null) {
                alert('区不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.street == null) {
                alert('街道不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.mobile == null) {
                alert('手机号不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.age == null) {
                alert('年龄不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.idcard == null) {
                alert('身份证号不能为空');
                return false;
            }

            if($scope.volunteerUpdateInfo.remarks == null) {
                alert('备注不能为空');
                return false;
            }
