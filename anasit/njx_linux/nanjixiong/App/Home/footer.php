	...
	<script type="text/javascript">
		...
		$('#comments_publish span').click(function(){
			var lvl=$(this).index();
			console.log(lvl);
			$.get('App/Home/ajax/exe.php?action=add_remark&level='+lvl+'&uid=<?php echo $jsuid; ?>&iid=<?php echo $jsiid; ?>',function(res){
				if(res=='-1'){
					alert('提交失败,请检查网络连接');
				}else{
					alert("提交成功,你提交了"+lvl+"分");
					$('#comments_publish').html('您已评分:'+lvl+'分');
				}
			});
		});
